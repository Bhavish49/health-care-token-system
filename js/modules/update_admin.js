...
const adminHtmlPath = 'c:\\healthCareTokenSystem\\dashboards\\admin.html';
...
const newStyles = `
        ...
        .header-top p { font-size: 0.85rem; color: #64748b; }
        ...
        .hero-img {
            background: url('https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?auto=format&fit=crop&w=800&q=80') center/cover;
            border-radius: 16px;
            min-height: 280px;
        }
        .hero-info {
        ...
        .metric-card h3 { font-size: 1.25rem; color: #fff; margin-bottom: 4px; font-family: 'Outfit'; }
        ...
        .btn-primary { background: var(--primary); color: #ffffff; padding: 10px 22px; border-radius: 8px; font-weight: 700; border: none; cursor: pointer; }
        ...
        <main class="main-content">
            ...
            <div id="section-doctors" class="hidden">
                <header style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 40px;">
                    <h2 style="font-size: 2rem;">Staff Directory</h2>
                    <button class="btn-primary" style="padding: 10px 20px;" onclick="promptInvite()">Invite Doctor</button>
                </header>
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 24px;" id="doctorGrid"></div>
            </div>
        </main>
    </div>
`;

// Now replace the <style> and <div class="dashboard-layout"> blocks
let newContent = content.replace(/<style>[\s\S]*?<\/style>/, '<style>\n' + newStyles + '\n    </style>');
newContent = newContent.replace(/<div class="dashboard-layout">[\s\S]*?<!-- MOBILE BOTTOM NAV -->/, newBody + '\n\n    <!-- MOBILE BOTTOM NAV -->');

fs.writeFileSync(adminHtmlPath, newContent, 'utf-8');
